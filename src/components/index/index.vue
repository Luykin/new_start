  ...
  <transition name="list">
    <div>
      ...
      <div class="header flex" style="border-top: 1.5px solid #10111E; height: 40px; align-items: flex-start;">
        ...
        <div class="flex" style="width: 80%; margin: 0 auto; overflow: hidden; justify-content: flex-start;">
          <div class="message-item-warp">
            <div v-for="item in message" class="message-item flex js"><img :src="item.avatar" class="message-avater" @error="_changurl($event)">恭喜<span style="color:#FFB800;margin: 0 5px;">{{item.nickname}}</span>盈利<span style="color:#FFB800;margin: 0 5px;">{{item.score}}幣-BTC</span></div>
          </div>
        </div>
      </div>
      ...
      <div class="header flex afterh hp1 ss">
        ...
        <p class="hfah-title double">预计收益: <span style="text-indent: 10px; color: #FFB800;">{{nowtrade.toFixed(2)}}BTC幣</span></p>
      </div>
      ...
      <popup ref="popup">
        <div class="result flex fw">
          ...
          <div class="result-list flex fw">
            <div class="result-item flex" v-for="item in transactionreualts" :class="{'active-tran': item.result == 1}">
              ...
              <div class="flex result-item-half fw ss">
                ...
                <p class="flex mg10 js textindex20">返还金额: <span class="color">{{item.result == 2 ? 0 : item.score + item.ret_score}}BTC币</span></p>
              </div>
            </div>
          </div>
      ...
      <div v-if="coursestep == 1" class="flex allscreen-course">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AC%AC%E4%B8%80%E6%AD%A5@3x.png" class="peple-course one-course" @click="_next">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AE%AD%E5%A4%B4@3x.png" class="arrow one-arrow">
      </div>
      <div v-if="coursestep == 2" class="flex allscreen-course">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AC%AC%E4%BA%8C%E6%AD%A5@3x.png" class="peple-course two-course" @click="_next">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AE%AD%E5%A4%B4@3x.png" class="arrow two-arrow">
      </div>
      <div v-if="coursestep == 3" class="flex allscreen-course">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AC%AC%E4%B8%89%E9%83%A8@3x.png" class="peple-course tir-course" @click="_next">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AE%AD%E5%A4%B4@3x.png" class="arrow tir-arrow">
      </div>
      <div v-if="coursestep == 4" class="flex allscreen-course">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AC%AC%E5%9B%9B%E6%AD%A5@3x.png" class="peple-course fow-course" @click="_next">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AE%AD%E5%A4%B4@3x.png" class="arrow fow-arrow">
      </div>
      <div v-if="coursestep == 5" class="flex allscreen-course">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AC%AC%E4%BA%94%E6%AD%A5@3x.png" class="peple-course" @click="_next">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AE%AD%E5%A4%B4@3x.png" class="arrow fiw-arrow">
      </div>
      <div v-if="coursestep == 6" class="flex allscreen-course">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AC%AC%E5%85%AD%E6%AD%A5@3x.png" class="peple-course" @click="_next">
        <img src="http://pd70b9zd0.bkt.clouddn.com/%E7%AE%AD%E5%A4%B4@3x.png" class="arrow six-arrow">
      </div>
      <img src="http://p70pqu6ys.bkt.clouddn.com/%E6%8F%90%E7%A4%BA@3x.png" style="position: absolute; z-index: -999; opacity: 0; width: 0;height: 0;left: -100px;top: -100px;" @load="_initall($event)">
    </div>
  </transition>
</template>
<script type="text/javascript">
  ...
  export default {
    ...
    created() {
      ...
      if (hours < 1) {
        this.peple = 4000 + Math.round(Math.random() * 2000)
      }
      if (hours >= 1 && hours <= 8) {
        this.peple = 200 + Math.round(Math.random() * 800)
      }
      if (hours > 8 && hours<=11) {
        this.peple = 1000 + Math.round(Math.random() * 1000)
      }
      if (hours > 12 && hours<=14) {
        this.peple = 3000 + Math.round(Math.random() * 1000)
      }
      if (hours > 15 && hours<=17) {
        this.peple = 2000 + Math.round(Math.random() * 1000)
      }
      if (hours > 18 && hours<= 24) {
        this.peple = 5000 + Math.round(Math.random() * 4000)
      }
    ...
    methods: {
      ...
      _closekefu() {
        ...
        // console.log(parseInt(localStorage.getItem('cousertime')) + 86400000, new Date().valueOf())
      ...
      _next() {
        this.coursestep += 1
        if (this.coursestep > 6) {
          localStorage.setItem('cousertime', new Date().valueOf())
          this.$refs.interlayer._hiddenLayer()
        }
      },
      _changurl(e) {
        if (e.target.src == DEFULAT_URL) {
          return false
        }
        e.target.src = DEFULAT_URL
      },
      _closeresult() {
        this.transactionreualts = []
      ...
      _initstarttimer() {
        ...
        let timer = setTimeout(() => {
          if (this.scale > 65 || randomnum > 0.5) {
            ...
            this.peple = Math.abs(parseInt(this.peple - randomnum*20))
          } else {
            this.scale = parseInt(this.scale + randomnum*5)
            this.peple = Math.abs(parseInt(this.peple + randomnum*20))
          }
          ...
          // this.peple = parseInt(this.peple + push*randomnum*20)
          this._initstarttimer()
          clearTimeout(timer)
        }, time)
      },
      _initevetbut() {
        this.$root.eventHub.$on('result', (data) => {
          ...
          this.transactionreualts.push(data.transactions)
      ...
      async _transaction(direction){
        if (!this.$root.user || !this.nowtrade) {
          this.$root.eventHub.$emit('titps', '登录中...')
          return false
        }
        if (this.$root.user.score < this.nowtrade + this.nowtrade * 0.1) {
        ...
        const ret = await transaction(direction, this.$root.user.username, this.nowtrade)
        if (ret.status == 200 && ret.data.code == 200) {
